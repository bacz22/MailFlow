...
export const UsageQuotaWidget: React.FC<UsageQuotaWidgetProps> = ({
  ...
  const contactThreshold = getThresholdState(contactsUsage.current, contactsUsage.limit)
    ...
    <Card className="border border-slate-200/90 dark:border-slate-800/90">
      <CardHeader className="pb-3 border-b border-slate-100 dark:border-slate-800">
        ...
        <CardDescription className="text-xs">
          Theo dõi số lượng email đã gửi và dung lượng danh bạ được lưu trữ trong tháng.
        </CardDescription>
      </CardHeader>

      <CardContent className="p-5 space-y-6">
        {/* 1. EMAILS USAGE PROGRESS BAR */}
        <div className="space-y-2 text-xs">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
            <div className="flex items-center gap-2 font-bold text-slate-900 dark:text-slate-100">
              <Mail className="w-4 h-4 text-blue-600" />
              <span>Lượng Email Đã Gửi Trong Tháng</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 text-[11px] font-bold">
                {emailThreshold.icon}
                <span className={emailThreshold.variant === 'danger' ? 'text-rose-600' : emailThreshold.variant === 'warning' ? 'text-amber-600' : 'text-slate-500'}>
          ...
          <div className="flex items-center justify-between text-[11px] text-slate-400">
            <span>Còn lại: <strong>{(emailsUsage.limit - emailsUsage.current).toLocaleString()}</strong> email</span>
            <span>Tự động làm mới vào 00:00 ngày {emailsUsage.resetDate}</span>
          </div>
        </div>

        {/* 2. CONTACTS STORAGE PROGRESS BAR */}
        <div className="space-y-2 text-xs">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
            <div className="flex items-center gap-2 font-bold text-slate-900 dark:text-slate-100">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 text-[11px] font-bold">
                ...
                <span className={contactThreshold.variant === 'danger' ? 'text-rose-600' : contactThreshold.variant === 'warning' ? 'text-amber-600' : 'text-slate-500'}>
          ...
          <div className="flex items-center justify-between text-[11px] text-slate-400">
            <span>Khả dụng thêm: <strong>{(contactsUsage.limit - contactsUsage.current).toLocaleString()}</strong> liên hệ</span>
            <span>Chỉ tính các liên hệ trạng thái Subscribed</span>
          </div>
        </div>

        {/* Warning Alert if near or exceeded */}
        {(emailThreshold.type !== 'normal' || contactThreshold.type !== 'normal') && (
          <div className="p-3.5 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900/60 flex items-center justify-between gap-3 text-xs text-amber-900 dark:text-amber-200 animate-in fade-in-0">
            <div className="flex items-center gap-2">
              ...
              <span>
                Bạn đã sử dụng trên <strong>80%</strong> hạn ngạch. Hãy cân nhắc nâng gói để không bị gián đoạn phân phối thư.
              </span>
            </div>
            {onUpgradeClick && (
              <button
